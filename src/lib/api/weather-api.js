...
class WeatherAPI {
  ...
  async getWeather(
    ...
    const forecast =
      forecastMode === "daily"
        ...
        : this._processHourlyForecast(rawData.hourly, rawData.current.time, timeFormat);
  ...
  getCachedWeather(
    ...
    const forecast =
      forecastMode === "daily"
        ...
        : this._processHourlyForecast(cached.data.hourly, cached.data.current.time, timeFormat);
  ...
  _getCachedData(latitude = null, longitude = null) {
    try {
      ...
      const { data, timestamp, latitude: cachedLat, longitude: cachedLon } = JSON.parse(cached);
  ...
  _coordinatesChanged(oldLat, oldLon, newLat, newLon) {
    const threshold = 0.1;
    return Math.abs(oldLat - newLat) > threshold || Math.abs(oldLon - newLon) > threshold;
  }

  /**
   * Private method to fetch raw weather data from API
   */
  async _fetchWeatherData(latitude, longitude, tempUnit, speedUnit, forecastMode = "hourly") {
    const baseParams = {
  ...
  _processCurrentWeather(currentData) {
    return {
      ...
      description: this._getWeatherDescription(currentData.weather_code, currentData.is_day === 1),
    };
  }
  ...
  _processHourlyForecast(hourlyData, currentTime, timeFormat = "12hr") {
    ...
    for (let i = 0; i < 5 && currentIndex + (i + 1) * 3 < hourlyData.time.length; i++) {
  ...
  _getWeatherDescription(weatherCode, isDay = true) {
    ...
    return descriptions[weatherCode]?.[timeOfDay]?.description?.toLowerCase() || "unknown";
  }
