...
class GoogleTasksBackendExtension extends TaskBackend {
  ...
  async getAuthToken(interactive = false) {
    if (!isChrome()) {
      throw new Error("Chrome identity API not available. Google Tasks only works in Chrome.");
    }
  ...
  async sync(resourceTypes = ["tasklists", "tasks"]) {
    try {
      ...
      if (resourceTypes.includes("tasklists")) {
        ...
        const hasValidTasklist = newTasklists.some((tl) => tl.id === this.defaultTasklistId);
      ...
      if (resourceTypes.includes("tasks")) {
        ...
        const completedThreshold = new Date(Date.now() - 24 * 60 * 60 * 1000);
  ...
  async completeTask(taskId) {
    ...
    const result = await this.apiRequest(`/lists/${tasklistId}/tasks/${taskId}`, {
      method: "PATCH",
      body: JSON.stringify({
        status: "completed",
