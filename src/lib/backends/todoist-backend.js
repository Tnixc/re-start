...
class TodoistBackend extends TaskBackend {
   ...
   */
  async sync(resourceTypes = ["items", "labels", "projects"], isRetry = false) {
  ...
  mergeData(type, newData) {
    ...
    newData.forEach((newItem) => {
      if (newItem.is_deleted) {
        this.data[type] = this.data[type].filter((item) => item.id !== newItem.id);
      } else {
        const existingIndex = this.data[type].findIndex((item) => item.id === newItem.id);
        if (existingIndex >= 0) {
  ...
  getLabelNames(labelIds) {
    ...
    return labelIds.map((id) => this.data.labels.find((l) => l.id === id)?.name).filter(Boolean);
  }
