...
function pushMonthMatch(match, monthWord, dayToken, yearToken, now, consider, defaultDay = null) {
...
function scoreCandidate(candidate) {
  const typeWeight = candidate.dateProvided === false ? 1 : candidate.hasTime ? 3 : 2;
...
export function parseSmartDate(input, maybeNow, maybeOptions) {
  ...
  } else if (typeof maybeNow === "number" || (typeof maybeNow === "string" && maybeNow)) {
    const candidate = new Date(maybeNow);
    if (!Number.isNaN(candidate.getTime())) now = candidate;
